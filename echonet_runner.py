...
class EchoNetRunner:
    ...
    def _run_task(self, task: dict, dcm_path: str, img_id: str, task_id: str, work_root: str | None, gpu_device: str | None = None) -> tuple:
        ...
        timings_path = None
        if task["script"] == "inference_2D_image.py":
            timings_path = os.path.join(out_dir, "stage-timings.json")
            cmd += [
        ...
        if timings_path and os.path.isfile(timings_path):
            self._update_plax_timing_summary(out_dir, timings_path)
        csv_path = out_file.replace(ext, "_distance.csv") if ext == ".avi" else None
        return csv_path, result.stdout

    @staticmethod
    def _update_plax_timing_summary(out_dir: str, timings_path: str) -> None:
        """Atomically collect per-model timing records beside the model directories."""
        echo_dir = Path(out_dir).parent
        target = echo_dir / "plax-stage-timings.json"
        records = []
        for timing_file in sorted(echo_dir.glob("*/stage-timings.json")):
            try:
                records.append(json.loads(timing_file.read_text(encoding="utf-8")))
            except (OSError, ValueError, TypeError):
                logger.warning("忽略损坏的阶段计时 path=%s", timing_file)
        temporary = target.with_name(target.name + f".{os.getpid()}.tmp")
        try:
            temporary.write_text(
                json.dumps({"schema_version": 1, "models": records}, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
