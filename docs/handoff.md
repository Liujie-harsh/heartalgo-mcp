...
- [x] 在启动服务的同一 PowerShell 中注入新清单并重启，确认启动日志及新 ECG 任务均显示 `heart@bundle-f06f5f050c98+models@4bf76612484dd393`。
- [x] 回归空收缩峰样本 `00005_851deaabbf3089ae.dcm`，确认 completed、六项结果且无 `IndexError`。
- [x] 对 AorticRoot 活动子进程做单模型/整任务性能采样，保存六项 `duration_seconds`、总耗时和进程树峰值 RSS，再决定 GPU 迁移方案。
- [x] 使用去标识化 DTD/实体和伪 XML 验证上传仍被 422 拒绝。
- [ ] 核对 PhysicalDelta 标尺问题，修正并以人工测量金标准验证 cm/cm·s⁻¹ 换算（临床上线阻塞项）。
- [x] 获取 Subcostal、RVOT、AV_Vmax 测试 DICOM，取消临时豁免并完成 12/12 门禁。
- [x] 确定 LA 切面口径（PLAX vs A4C），确认用PLAX。
